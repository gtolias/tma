...
# clone file from CIR with added CroW support
...
def crow(x):
    # transfer to numpy
    X = x.cpu().data.squeeze(0).numpy()

    # spatial weights
    a, b = 2, 2
    S = X.sum(axis=0)
    z = (S**a).sum()**(1./a)
    S =  (S / z)**(1./b) if b != 1 else (S / z)
    ...
    # transfer to torch and return
    return torch.from_numpy(X).unsqueeze(0).cuda()
